...
from Clearscrape.items import Site
from domaincount import DomainCount


class ClearSpider(CrawlSpider):
    """
    A web-mining spider.

    The ClearSpider opens a file containing a list of URLs, and crawls through
    ...
    of the session, although links may still be followed. The ClearSpider
    downloads the full HTML of every page it visits, as well as noting all
    external links on each page.

    To use this spider: scrapy crawl clear -a sites=startingListURLS.txt
    """
    name = 'clear'
    handle_httpstatus_list = [301]
    def __init__(self, sites=None, tor=False, db=False, *args, **kwargs):
        """
        Initializes the ClearSpider with a file of sites
        ...
        :param kwargs:
        """
        super(ClearSpider, self).__init__(*args, **kwargs)
        try:
            f = open("%s" % sites, "r")
            ...
            os._exit(1)
        #commenting this out for now to try and accommodate clearnet crawling (RV 12-10)
